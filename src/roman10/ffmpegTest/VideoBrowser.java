...
public class VideoBrowser extends ListActivity implements ListView.OnScrollListener {
	...
	static {
    	...
    	System.loadLibrary("encoding-example");
    }
	...
	private static native void naDecodeVideo(String pOutFileName, String pFileName);
		...
		if (selectedFileOrDirName.equals(upOneLevel)) {
			...
			if (l_clickedFile != null) {
				if (l_clickedFile.isDirectory()) {
					...
					String outFileName = new String(Environment.getExternalStorageDirectory().getPath() + "/decoded.mpeg4");
					
					//naDecodeVideo(outFileName, l_clickedFile.getAbsolutePath());
				}
			}
		}
	}
